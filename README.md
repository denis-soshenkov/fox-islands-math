...
2. На сервере положите его одной строкой в `/opt/fox-islands/deploy/.env`
   (рядом с compose-файлом — именно оттуда Docker Compose читает переменные):
...
| `pages` | Резервное зеркало на GitHub Pages (github.io); прод живёт на VPS |
